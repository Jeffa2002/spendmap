...
if (!sessionSecret || !encryptionKey || encryptionKey.length < 32) throw new Error('Missing secure Spendmap runtime secrets');
const key = crypto.createHash('sha256').update(encryptionKey).digest();
const hash = value => crypto.createHash('sha256').update(value).digest('hex');
const id = () => crypto.randomUUID();
const now = () => Date.now();

function encrypt(value) { const iv = crypto.randomBytes(12); const cipher = crypto.createCipheriv('aes-256-gcm', key, iv); const out = Buffer.concat([cipher.update(value, 'utf8'), cipher.final()]); return `${iv.toString('base64url')}.${cipher.getAuthTag().toString('base64url')}.${out.toString('base64url')}`; }
function decrypt(value) { const [iv, tag, out] = value.split('.').map(v => Buffer.from(v, 'base64url')); const decipher = crypto.createDecipheriv('aes-256-gcm', key, iv); decipher.setAuthTag(tag); return Buffer.concat([decipher.update(out), decipher.final()]).toString('utf8'); }
